...
if [[ "${choices["btop (modern top replacement)"]}" == "true" ]]; then
    if [[ -n "${TERMUX_VERSION:-}" ]] || [[ "${PREFIX:-}" == *"com.termux"* ]] || [[ -d "/data/data/com.termux/files/usr" ]]; then
        pkg install -y htop && echo "→ btop installed"
    else
        sudo apt install -y btop && echo "→ btop installed"
    fi
fi
[[ "${choices["yazi (terminal file manager)"]}" == "true" ]] && bash "$INSTALL_DIR/includes/install_yazi.sh"